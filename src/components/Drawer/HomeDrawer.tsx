...
import { OverViewIcon } from "../customSvgs/CustomSvgs";
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      >
        ...
        <Divider />
        <Box className="menuList__box--top">
          <List>
            {["Overview", "Scholars", "Tracker", "Settings"].map(
              (text, index) => (
                <ListItem
                  sx={
                    pathname === "/dashboard" && text === "Overview"
                      ? activeRoute
                      : onHoverStyles
                  }
                  className="menuList__icons"
                  button
                  key={text}
                >
                  <ListItemIcon>
                    {index % 2 === 0 ? <OverViewIcon /> : <SettingsIcon />}
                  </ListItemIcon>
                  <div className="menuList__div--text">{text}</div>
